...
contract SwapHub {
    ...
    function swap(
        ...
        uint256 amountOut = swapInternal(cache, swapHandler, params);
    ...
    function swapAndRepay(
        ...
        uint256 owed = IEVault(eTokenOut).debtOf(accountOut);
    ...
    function swapInternal(SwapCache memory cache, address swapHandler, ISwapHandler.SwapParams memory params)
        private
        returns (uint256)
    {
        // Supply handler, for exact output amount transfered serves as an implicit amount in max.
        if (params.amountIn == type(uint256).max) {
            params.amountIn = IEVault(cache.eTokenIn).maxWithdraw(cache.accountIn);
        }
        ...
        uint256 postBalanceIn = IERC20(params.underlyingIn).balanceOf(address(this));
        uint256 postBalanceOut = IERC20(params.underlyingOut).balanceOf(address(this));
        ...
        if (params.mode == 0) {
            ...
            unchecked {
                amountOutMin = params.amountOut - params.exactOutTolerance;
            }
        }
