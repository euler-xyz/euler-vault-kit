...
abstract contract UsdxlMintingModule is IBorrowing, AssetTransfers, BalanceUtils, LiquidityUtils {
    ...
    function borrow(uint256 amount, address receiver) public virtual nonReentrant returns (uint256) {
        ...
        //TODO: call cash() which will check facilitator bucket for this vault on USDXL
        if (assets > vaultCache.cash) revert E_InsufficientCash();

        increaseBorrow(vaultCache, account, assets);

        //TODO: mint USDXL to receiver; no need to update vaultCache.cash
    ...
    function repay(uint256 amount, address receiver) public virtual nonReentrant returns (uint256) {
        ...
        //TODO: pull and burn USDXL from account
...
contract UsdxlMinting is UsdxlMintingModule {
