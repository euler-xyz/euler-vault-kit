...
contract SwapHandlerUniAutoRouter is SwapHandlerCombinedBase {
    ...
    constructor(address uniSwapRouter02_, address uniSwapRouterV2, address uniSwapRouterV3)
    ...
    function swapPrimary(SwapParams memory params) internal override returns (uint256 amountOut) {
        ...
        if (params.mode == 0) {
            ...
            uint256 preBalance = IERC20(params.underlyingOut).balanceOf(msg.sender);

            swapInternal(params);

            uint256 postBalance = IERC20(params.underlyingOut).balanceOf(msg.sender);
