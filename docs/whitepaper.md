...
Nested vaults follow the same principle, however they are more convenient to use because of EVC batching and flash liquidity, and are more gas-efficient than interacting with entirely separate systems. Euler Vaults have been designed to allow nesting for all viable configurations without triggering re-entrancy problems, and `convertToAssets` can be safely used as a [pricing oracle](#pricing-shares). However, there's an exception where nesting must not be used: when nested vault is used as collateral for the liability vault. Such a configuration will trigger a re-entrancy which will prevent the liquidation from happening.
